...
from source_sftp_bulk_encoding.client import SFTPClient
from source_sftp_bulk_encoding.spec import SourceSFTPBulkSpec
...
class SourceSFTPBulkStreamReader(AbstractFileBasedStreamReader):
    ...
    def open_file(self, file: RemoteFile, mode: FileReadMode, encoding: Optional[str], logger: logging.Logger) -> IOBase:
        logger.info(f"Opening file {file.uri} using encoding {encoding}")
        remote_file_raw = BytesIO()
        self.sftp_client.sftp_connection.getfo(file.uri, remote_file_raw)
        remote_file_raw.seek(0)
        remote_file = TextIOWrapper(remote_file_raw, encoding=encoding)
        return remote_file
